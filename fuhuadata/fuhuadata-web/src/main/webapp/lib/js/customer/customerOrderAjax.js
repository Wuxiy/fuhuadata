...
function customerOrderHis(result,pn,pz) {
    ...
    if(result && result!='' && result!=null){
        ...
        $.each(getData,function (n,item) {
            ...
            tr += '<td><a href="">'+(item.orderId?item.orderId:'')+'</a></td>';
            ...
            tr += '<td>'+(item.isComplaint?arr[item.isComplaint]:'')+'</td>';//返回值所对应的字符串
            tr += '<td>'+(item.isModifyPrice?arr[item.isModifyPrice]:'')+'</td>';//返回值所对应的字符串
            tr += '<td>'+(item.netProfit?item.netProfit:'')+'</td>';
            tr += '<td>'+(item.isPledge?arr[item.isPledge]:'')+'</td>';//返回值所对应的字符串
            tr += '<td>'+(item.dealTime?item.dealTime.split(' ')[0]:'')+'</td>';//只保留日期
            tr += '</tr>';
            tbody.append(tr);
        })
    }
